        ...
        try {
          if(req.body.logemail == result.email){
            //if(req.body.logpassword == result.userpassword){
              ...
              var db = req.db;
               db.get('annoncecollection').findOne({"user.us_id": result._id},(err, item) =>{
                //db.get('annoncecollection').find({},{},function(e, docs){
                  console.log('resultat : ' + item )
                res.render('dashboard',{"listannonceuser": ditemocs,"u_id": result._id, "u_name": result.username, "u_email": result.email, "u_typecompte": result.typecompte});
              });
