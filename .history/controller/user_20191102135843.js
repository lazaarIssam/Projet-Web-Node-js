...
/* GET login page */
  ...
  exports.login = function(req, res) {
    ...
    var collectionprofil = db.get('annoncecollection')
        ...
        try {
          if(req.body.logemail == result.email){
            //if(req.body.logpassword == result.userpassword){
              ...
              var db = req.db;
              db.get('annoncecollection').findOne({'user.us_id':result._id}).then( function(item){
              ...
              //res.render('dashboard',{"u_id": result._id, "u_name": result.username, "u_email": result.email, "u_typecompte": result.typecompte});
                ...
                .find({},function (e,result){
                  res.render('profil',{"result":result,"sess": us_email,"sesid": req.session.id,"sesname": req.session._id});
                });
