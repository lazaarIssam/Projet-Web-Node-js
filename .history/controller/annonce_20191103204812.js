...
exports.deleteann= function(req, res) {
  ...
  db.collection('annoncecollection').remove({"_id":newObjectId},function(err,data){
    ...
    db.collection('annoncecollection').find({"user.us_id":data.user.us_id},function(err,annonce){
      db.collection('usercollection').find({"_id":data.user.us_id},function(err,userr){
        res.render('dashboard',"listannonceuser": annonce,"u_id": result._id, "u_name": result.username, "u_email": result.email, "u_typecompte": result.typecompte);
      })
    })
  })
}
